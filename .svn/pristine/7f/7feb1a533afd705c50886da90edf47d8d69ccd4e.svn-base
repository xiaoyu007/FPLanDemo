...
@synthesize maximumNumberOfDatabasesToCreate =
...
+ (instancetype)databasePoolWithPath:(NSString *)aPath flags:(int)openFlags {
  return FMDBReturnAutoreleased(
      [[self alloc] initWithPath:aPath flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString *)aPath flags:(int)openFlags {
  ...
  if (self != nil) {
    _path = [aPath copy];
    _lockQueue = dispatch_queue_create(
        [[NSString stringWithFormat:@"fmdb.%@", self] UTF8String], NULL);
...
- (void)pushDatabaseBackInPool:(FMDatabase *)db {
  ...
  if (!db) { // db can be null if we set an upper bound on the # of databases to
             // create.
    return;
  }
  ...
  [self executeLocked:^() {
    ...
    if ([_databaseInPool containsObject:db]) {
      [[NSException exceptionWithName:@"Database already in pool"
                               reason:@"The FMDatabase being put back into the "
                                      @"pool is already present in the pool"
                             userInfo:nil] raise];
    }
    ...
    if (db) {
      ...
      if (_maximumNumberOfDatabasesToCreate) {
        NSUInteger currentCount =
            [_databaseOutPool count] + [_databaseInPool count];

        if (currentCount >= _maximumNumberOfDatabasesToCreate) {
          // nslog(@"Maximum number of databases (%ld) has already been
          // reached!", (long)currentCount);
          return;
        }
      }
...
// This ensures that the db is opened before returning
    ...
    if ([db open]) {
#endif
      if ([_delegate respondsToSelector:@selector(databasePool:
                                            shouldAddDatabaseToPool:)] &&
          ![_delegate databasePool:self shouldAddDatabaseToPool:db]) {
        [db close];
        db = 0x00;
      } else {
...
- (void)beginTransaction:(BOOL)useDeferred
               withBlock:(void (^)(FMDatabase *db, BOOL *rollback))block {
