...
@implementation AnimationLabelAlterView

static AnimationLabelAlterView *shareAlertViewInstance = nil;

+ (AnimationLabelAlterView *)shareManager {
...
+ (void)YouGu_animation_Did_Start:(NSString *)message {
  if (!message || [message length] == 0) {
    [[AnimationLabelAlterView shareManager] performSelector:@selector(hideNewShowLabel)
                        withObject:nil
                        afterDelay:2.0f];
    return;
  }
  ...
  if (messageSize.height > 20.0f) {
    [AnimationLabelAlterView shareManager].width = messageSize.width;
  } else {
    [AnimationLabelAlterView shareManager].width = messageSize.width + 26.0f;
  }
