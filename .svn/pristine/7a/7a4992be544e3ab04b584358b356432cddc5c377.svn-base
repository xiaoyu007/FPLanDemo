...
- (instancetype)initWithPath:(NSString *)aPath {
  ...
  assert(sqlite3_threadsafe()); // whoa there big boy- gotta make sure sqlite it
                                // happy with what we're going to do.
...
- (BOOL)openWithFlags:(int)flags {
  int err = sqlite3_open_v2([self sqlitePath], &_db, flags,
...
- (void)setBusyRetryTimeout:(int)i {
  // nslog(@"%s:%d", __FUNCTION__, __LINE__);
  // nslog(@"FMDB: setBusyRetryTimeout does nothing, please use
  // setRetryTimeout:");
}
...
- (void)closeOpenResultSets {
  ...
  for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
    FMResultSet *rs =
        (FMResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
  ...
  NSDateFormatter *result =
      FMDBReturnAutoreleased([[NSDateFormatter alloc] init]);
  result.dateFormat = format;
  result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
  result.locale = FMDBReturnAutoreleased(
...
- (BOOL)goodConnection {
  ...
  FMResultSet *rs =
      [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)bindObject:(id)obj toColumn:(int)idx inStatement:(sqlite3_stmt *)pStmt {
  ...
  // FIXME - someday check the return codes on these binds.
    ...
    if (!bytes) {
      ...
      // Don't pass a NULL pointer, or sqlite will bind a SQL null instead of a
      // blob.
      bytes = "";
    }
    ...
    if (strcmp([obj objCType], @encode(BOOL)) == 0) {
      ...
      sqlite3_bind_text(pStmt, idx, [[obj description] UTF8String], -1,
                        SQLITE_STATIC);
    }
  } else {
    sqlite3_bind_text(pStmt, idx, [[obj description] UTF8String], -1,
  ...
  for (NSUInteger i = 0; i < length; ++i) {
    ...
    if (last == '%') {
      ...
      case '@':
        ...
        break;
      case 'c':
        // warning: second argument to 'va_arg' is of promotable type 'char';
      ...
      case 'h':
        ...
        if (i < length && [sql characterAtIndex:i] == 'i') {
          ...
          //  'short'; this va_arg has undefined behavior because arguments will
          //  be promoted to 'int'
          ...
          arg = [NSNumber
              numberWithUnsignedShort:(unsigned short)(va_arg(args, uint))];
        } else {
          i--;
        }
        break;
      ...
      case 'l':
        ...
        break;
      default:
        // something else that we can't interpret. just pass it on through like
        // normal
        break;
      }
    } else if (current == '%') {
...
// nslog(@"DB Path: %@", _databasePath);
        ...
        if (_crashOnErrors) {
          abort();
          NSAssert2(false, @"DB Error: %d \"%@\"", [self lastErrorCode],
  ...
  id obj;
  int idx = 0;
  int queryCount = sqlite3_bind_parameter_count(
      pStmt); // pointed out by Dominic Yu (thanks!)

  // If dictionaryArgs is passed in, that means we are using sqlite's named
  // parameter support
  if (dictionaryArgs) {
    ...
    for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
      ...
      NSString *parameterName =
          [[NSString alloc] initWithFormat:@":%@", dictionaryKey];
  ...
  if (idx != queryCount) {
    // nslog(@"Error: the bind count is not correct for the # of variables
    // (executeQuery)");
    sqlite3_finalize(pStmt);
    _isExecutingStatement = NO;
    return nil;
...
- (FMResultSet *)executeQuery:(NSString *)sql, ... {
  ...
  id result = [self executeQuery:sql
            withArgumentsInArray:nil
                    orDictionary:nil
                        orVAList:args];
...
// nslog(@"DB Path: %@", _databasePath);
      ...
      if (outErr) {
        *outErr = [self
            errorWithMessage:[NSString
                                 stringWithUTF8String:sqlite3_errmsg(_db)]];
      }
  ...
  if (idx != queryCount) {
    // nslog(@"Error: the bind count (%d) is not correct for the # of variables
    // in the query (%d) (%@) (executeUpdate)", idx, queryCount, sql);
    sqlite3_finalize(pStmt);
    _isExecutingStatement = NO;
    return NO;
  }
  ...
  if (SQLITE_DONE == rc) {
    ...
    if (_logsErrors) {
      // nslog(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc,
      // sqlite3_errmsg(_db));
      // nslog(@"DB Query: %@", sql);
    }
    ...
    // uh oh.
    if (_logsErrors) {
      // nslog(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc,
      // sqlite3_errmsg(_db));
      // nslog(@"DB Query: %@", sql);
    }
  } else {
    // wtf?
    if (_logsErrors) {
      // nslog(@"Unknown error calling sqlite3_step (%d: %s) eu", rc,
      // sqlite3_errmsg(_db));
      // nslog(@"DB Query: %@", sql);
    }
  }
  ...
  if (rc == SQLITE_ROW) {
    NSAssert1(NO, @"A executeUpdate is being called with a query string '%@'",
              sql);
  }
  ...
  if (closeErrorCode != SQLITE_OK) {
    if (_logsErrors) {
      // nslog(@"Unknown error finalizing or resetting statement (%d: %s)",
      // closeErrorCode, sqlite3_errmsg(_db));
      // nslog(@"DB Query: %@", sql);
    }
  }
...
static NSString *FMEscapeSavePointName(NSString *savepointName) {
  return [savepointName stringByReplacingOccurrencesOfString:@"'"
                                                  withString:@"''"];
}
...
- (BOOL)startSavePointWithName:(NSString *)name error:(NSError **)outErr {
  ...
  NSString *sql = [NSString
      stringWithFormat:@"savepoint '%@';", FMEscapeSavePointName(name)];
...
- (BOOL)releaseSavePointWithName:(NSString *)name error:(NSError **)outErr {
  ...
  NSString *sql = [NSString
      stringWithFormat:@"release savepoint '%@';", FMEscapeSavePointName(name)];
...
- (BOOL)rollbackToSavePointWithName:(NSString *)name error:(NSError **)outErr {
  ...
  NSString *sql =
      [NSString stringWithFormat:@"rollback transaction to savepoint '%@';",
                                 FMEscapeSavePointName(name)];
  BOOL worked = [self executeUpdate:sql];
...
- (NSError *)inSavePoint:(void (^)(BOOL *rollback))block {
  ...
  NSString *name =
      [NSString stringWithFormat:@"dbSavePoint%ld", savePointIdx++];
...
void FMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc,
                                     sqlite3_value **argv);
void FMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc,
                                     sqlite3_value **argv) {
#if !__has_feature(objc_arc)
...
/* I tried adding custom functions to release the block when the connection is
 * destroyed- but they seemed to never be called, so we use _openFunctions to
 * store the values instead. */
#if !__has_feature(objc_arc)
  sqlite3_create_function([self sqliteHandle], [name UTF8String], count,
                          SQLITE_UTF8, (void *)b,
                          &FMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
  sqlite3_create_function([self sqliteHandle], [name UTF8String], count,
                          SQLITE_UTF8, (__bridge void *)b,
                          &FMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
...
- (NSString *)description {
  return [NSString stringWithFormat:@"%@ %ld hit(s) for query %@",
                                    [super description], _useCount, _query];
}
