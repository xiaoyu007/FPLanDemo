...
#import "SQLpraiseServer.h"

#define kUserTableName @"PUser_Commentlist"
#define kDefaultDBName @"NEWS_Collection_zan.sqlite"

static SQLpraiseServer *_SQLpraiseServer = nil;

@implementation SQLpraiseServer
  ...
  if ([[NSFileManager defaultManager]
          fileExistsAtPath:pathInCacheDirectory([NSString
                               stringWithFormat:@"/Collection.xmly/%@",
                                                kDefaultDBName])] == NO) {
    _SQLpraiseServer = nil;
  }
  @synchronized([SQLpraiseServer class]) {
    if (!_SQLpraiseServer)
      _SQLpraiseServer = [[self alloc] init];
    return _SQLpraiseServer;
  }
