 ...
 *     response -
 * 返回结果字典，包含：error_code/request_id/app_id/user_id/channel_id/error_msg
 * @return
 *     none
 */
 ...
 * @param
 *     deviceToken - 通过app
 * delegate的didRegisterForRemoteNotificationsWithDeviceToken回调的获取
 * @return
 *     none
 */
...
/**
 * 设置access token. 在bindChannel之前调用，如果access
 * token改变后，必须重新设置并且重新bindChannel
 * @param
 *     token - Access Token
 * @return
 *     none
...
#define BPushRequestResponseParamsKey                                          \
  @"response_params" // 服务端返回的原始值，其内容和以上的部分值可能有重合
