...
//yuling 新添加的
#define kRFRequestParamFileName @"kRFRequestParamFileName"
...
+(id) requestWithURL:(NSURL*)url type:(RFRequestMethod)t resourcePathComponents:(id)firstObject, ... 
{
  NSLog(@"current_request_url = %@", url);
	...
	return [[RFRequest alloc] initWithURL:url type:t resourcePath:[NSArray arrayWithArray:path]];
}
...
+(id) requestWithURL:(NSURL*)url type:(RFRequestMethod)t bodyContentType:(RFRequestBodyType)bt resourcePathComponents:(id)firstObject, ... 
{
	...
	return [[RFRequest alloc] initWithURL:url type:t resourcePath:path bodyContentType:bt];
}
...
#pragma mark - Methods  yuling，新添加的
-(void) addData:(NSData*)data  withFileName:(NSString *)fileName withContentType:(NSString*)contentType forKey:(NSString*)key {
	if (!self.params) {
         params=[[NSMutableArray alloc]initWithCapacity:0];
	}
	
	NSDictionary* pd = [NSDictionary dictionaryWithObjectsAndKeys:key, kRFRequestParamKey, data, kRFRequestParamValue, contentType, kRFRequestParamMIME, fileName,kRFRequestParamFileName, nil];
...
{
	//Request
	NSMutableURLRequest *urlRequest = [[NSMutableURLRequest alloc] initWithURL:[self URL] cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:30.0f];
    ...
    //nslog(@"%d",self.bodyData && self.bodyData.length > 0);
	...
	if (self.requestMethod != RFRequestMethodGet && bData && bData.length > 0) {
		[urlRequest setValue:[NSString stringWithFormat:@"%lu", (unsigned long)[bData length]] forHTTPHeaderField:@"Content-Length"];
...
-(NSData*) constructBody
{
	...
	else if(self.bodyContentType == RFRequestBodyTypeMultiPartFormData)
	{
		...
		for (NSDictionary* pd in self.params) {
			...
			[data appendData:[[NSString stringWithFormat:@"--%@", kRFPostBoundary] dataUsingEncoding:NSUTF8StringEncoding]];
			[data appendData:crlf];
            //nslog(@"pppooo:%@,%@",[pd objectForKey:kRFRequestParamKey],[pd objectForKey:kRFRequestParamFileName]);
        [data appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"; filename=\"%@\"", [pd objectForKey:kRFRequestParamKey],[pd objectForKey:kRFRequestParamFileName]] dataUsingEncoding:NSUTF8StringEncoding]];
            [data appendData:crlf];
			[data appendData:crlf];
