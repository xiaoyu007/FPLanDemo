...
#import "The_NEWS_Focus_SQL.h"

#define kUserTableName @"SUser_NEWS_Focus_list"
#define kDefaultDBName @"The_NEWS_NEWS_Focus.sqlite"

static The_NEWS_Focus_SQL *_the_NEWS_Focus_SQL = nil;

@implementation The_NEWS_Focus_SQL
  ...
  if ([[NSFileManager defaultManager]
          fileExistsAtPath:pathInCacheDirectory([NSString
                               stringWithFormat:@"/Collection.xmly/%@",
                                                kDefaultDBName])] == NO) {
    _the_NEWS_Focus_SQL = nil;
  }
  @synchronized([The_NEWS_Focus_SQL class]) {
    if (!_the_NEWS_Focus_SQL)
      _the_NEWS_Focus_SQL = [[self alloc] init];
    return _the_NEWS_Focus_SQL;
  }
...
+ (id)alloc {
  ...
  @synchronized([The_NEWS_Focus_SQL class]) {
    NSAssert(_the_NEWS_Focus_SQL == nil,
             @"Attempted to allocated a second instance_SQLDataHtmlstring");
    _the_NEWS_Focus_SQL = [super alloc];
    return _the_NEWS_Focus_SQL;
  }
  return nil;
}
...
- (void)createDataBase {
  ...
  if ([set intForColumnIndex:0]) {
    ...
    // TODO: 插入新的数据库
    [_db executeUpdate:@"CREATE TABLE SUser_NEWS_Focus_list (uid INTEGER "
...
- (void)saveUser:(NSString *)name
  ...
  if (name && channlid && description) {
    NSMutableString *query =
        [NSMutableString stringWithFormat:@"INSERT INTO SUser_NEWS_Focus_list"];
...
- (void)deleteUserWith_ALL:(NSArray *)uids {
  NSString *query =
      [NSString stringWithFormat:@"DELETE FROM SUser WHERE uid in ("];
 ...
 */
- (NSArray *)findWithUid {
  //    NSString * query = @"SELECT * FROM SUser_NEWS_Focus_list ORDER BY uid
  //    DESC";

  FMResultSet *rs = [_db
      executeQuery:@"SELECT * FROM SUser_NEWS_Focus_list ORDER BY uid DESC"];
