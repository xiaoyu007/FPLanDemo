...
- (id)initWithNamespace:(NSString *)ns {
  if ((self = [super init])) {
    NSString *fullNamespace =
        [@"com.hackemist.SDWebImageCache." stringByAppendingString:ns];
...
- (void)storeImage:(UIImage *)image
  ...
  [self.memCache setObject:image
                    forKey:key
                      cost:image.size.height * image.size.width * image.scale];
  ...
  if (toDisk) {
    dispatch_async(self.ioQueue, ^{
      ...
      if (data) {
        ...
        [fileManager createFileAtPath:[self defaultCachePathForKey:key]
                             contents:data
                           attributes:nil];
      }
    });
...
- (UIImage *)imageFromDiskCacheForKey:(NSString *)key {
  ...
  if (diskImage) {
    CGFloat cost =
        diskImage.size.height * diskImage.size.width * diskImage.scale;
...
- (UIImage *)imageFromMemoryOrDiskCacheForKey:(NSString *)imageUrl {
  //读内存
  UIImage *cachedImage = [self imageFromMemoryCacheForKey:imageUrl];
  if (!cachedImage) {
    //读磁盘
    cachedImage = [self imageFromDiskCacheForKey:imageUrl];
  }
  ...
  if (fromDisk) {
    dispatch_async(self.ioQueue, ^{
      [[NSFileManager defaultManager]
          removeItemAtPath:[self defaultCachePathForKey:key]
                     error:nil];
    });
  }
}
...
- (void)clearDisk {
  dispatch_async(self.ioQueue, ^{
    [[NSFileManager defaultManager] removeItemAtPath:self.diskCachePath
                                               error:nil];
...
- (void)cleanDisk {
  dispatch_async(self.ioQueue, ^{
    ...
    NSDirectoryEnumerator *fileEnumerator =
        [fileManager enumeratorAtURL:diskCacheURL
            includingPropertiesForKeys:resourceKeys
                               options:NSDirectoryEnumerationSkipsHiddenFiles
                          errorHandler:NULL];

    NSDate *expirationDate =
        [NSDate dateWithTimeIntervalSinceNow:-self.maxCacheAge];
    ...
    for (NSURL *fileURL in fileEnumerator) {
      NSDictionary *resourceValues =
          [fileURL resourceValuesForKeys:resourceKeys error:NULL];
    ...
    if (self.maxCacheSize > 0 && currentCacheSize > self.maxCacheSize) {
      ...
      // Sort the remaining cache files by their last modification time (oldest
      // first).
...
- (unsigned long long)getSize {
  ...
  NSDirectoryEnumerator *fileEnumerator =
      [[NSFileManager defaultManager] enumeratorAtPath:self.diskCachePath];
...
- (void)calculateSizeWithCompletionBlock:
        (void (^)(NSUInteger fileCount,
                  unsigned long long totalSize))completionBlock {
  NSURL *diskCacheURL =
      [NSURL fileURLWithPath:self.diskCachePath isDirectory:YES];
