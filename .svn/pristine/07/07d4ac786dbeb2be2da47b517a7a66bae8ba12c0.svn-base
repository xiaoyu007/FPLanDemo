...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `FMDatabase` - Represents a single SQLite database.  Used for executing SQL
 statements.
 - `<FMResultSet>` - Represents the results of executing a query on an
 `FMDatabase`.
 - `<FMDatabaseQueue>` - If you want to perform queries and updates on multiple
 ...
 @warning Do not instantiate a single `FMDatabase` object and use it across
 multiple threads. Instead, use `<FMDatabaseQueue>`.
...
/** Create a `FMDatabase` object.
 ...
 An `FMDatabase` is created with a path to a SQLite database file.  This path
 ...
 location.  This database is deleted with the `FMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed
 with the `FMDatabase` connection is closed.
...
/** Opening a new database connection with flags
 ...
 @param flags one of the following three values, optionally combined with the
...
/** Execute update statement
 ...
 This method employs [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 ...
 handles `NSString`, `NSNumber`, `NSNull`, `NSDate`, and `NSData` objects. All
 other object types will be interpreted as text values using the object's
 `description` method.

 @param sql The SQL to be performed, with optional `?` placeholders.

 @param outErr A reference to the `NSError` pointer to be updated with an auto
 released `NSError` object if an error if an error occurs. If `nil`, no
 `NSError` object will be returned.

 @param ... Optional parameters to bind to `?` placeholders in the SQL
 statement. These should be Objective-C objects (e.g. `NSString`, `NSNumber`,
 etc.), not fundamental C data types (e.g. `int`, `char *`, etc.).

 @return `YES` upon success; `NO` upon failure. If failed, you can call
 `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic
...
/** Execute update statement
 ...
 Any sort of SQL statement which is not a `SELECT` statement qualifies as an
 update.  This includes `CREATE`, `UPDATE`, `INSERT`, `ALTER`, `COMMIT`,
 `BEGIN`, `DETACH`, `DELETE`, `DROP`, `END`, `EXPLAIN`, `VACUUM`, and `REPLACE`
 statements (plus many more).  Basically, if your SQL statement does not begin
 with `SELECT`, it is an update statement.

 @param format The SQL to be performed, with `printf`-style escape sequences.

 @param ... Optional parameters to bind to use in conjunction with the
 ...
 @warning This should be used with great care. Generally, instead of this
 method, you should use `<executeUpdate:>` (with `?` placeholders in the SQL),
 which properly escapes quotation marks encountered inside the values
 (minimizing errors and protecting against SQL injection attack) and handles a
 wider variety of data types. See `<executeUpdate:>` for more information.
 */
...
/** Execute update statement
 ...
 @param arguments A `NSArray` of objects to be used when binding values to the
...
/** Execute update statement
 ...
 @param arguments A `NSDictionary` of objects keyed by column names that will be
 used when binding values to the `?` placeholders in the SQL statement.
...
/** Last insert rowid
 ...
 database from the database connection in the first argument. As of SQLite
 version 3.7.7, this routines records the last insert rowid of both ordinary
 tables and virtual tables. If no successful `INSERT`s have ever occurred on
...
/** Execute select statement
 ...
 Executing queries returns an `<FMResultSet>` object if successful, and `nil`
 upon failure.  Like executing updates, there is a variant that accepts an
 `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and
 `<lastErrorMessage>` methods to determine why a query failed.
 ...
 If failed, you can call `<lastError>`, `<lastErrorCode>`, or
...
/** Execute select statement
 ...
 @warning This should be used with great care. Generally, instead of this
 method, you should use `<executeQuery:>` (with `?` placeholders in the SQL),
 which properly escapes quotation marks encountered inside the values
 (minimizing errors and protecting against SQL injection attack) and handles a
 wider variety of data types. See `<executeQuery:>` for more information.
...
/** Begin a transaction
 ...
 @return `YES` on success; `NO` on failure. If failed, you can call
 `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic
 information regarding the failure.
...
/** Test to see if the library is threadsafe
 ...
 @return Zero if and only if SQLite was compiled with mutexing code omitted due
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL
 ...
 For example:
    ...
    [queue inDatabase:^(FMDatabase *adb) {
        ...
        [adb makeFunctionNamed:@"StringStartsWithH" maximumArguments:1
 withBlock:^(sqlite3_context *context, int aargc, sqlite3_value **aargv) {
            if (sqlite3_value_type(aargv[0]) == SQLITE_TEXT) {
                ...
                //nslog(@"Unknown formart for StringStartsWithH (%d) %s:%d",
 sqlite3_value_type(aargv[0]), __FUNCTION__, __LINE__);
                sqlite3_result_null(context);
            }
        }];

        int rowCount = 0;
        FMResultSet *ars = [adb executeQuery:@"select * from ftest where
 StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of
 ...
 @return A `NSDateFormatter` that can be used for converting dates to strings
 and vice versa.
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter
 generated by this method should be assigned to only one FMDB instance and
...
/** Set to a date formatter to use string dates with sqlite instead of the
 ...
 using a formatter generated using FMDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you
 should not use the formatter you pass to FMDB for other purposes, as
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you
 will not need to interact directly with `FMStatement`, but rather with
 `<FMDatabase>` and `<FMResultSet>` only.
