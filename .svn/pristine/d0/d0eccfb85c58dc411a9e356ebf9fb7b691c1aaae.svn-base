...
#define kPRBGColor                                                                                 \
  [UIColor colorWithRed:226.0 / 255.0 green:231.0 / 255.0 blue:237.0 / 255.0 alpha:0.0]
...
#pragma mark - Default is at top
// Default is at top
- (id)initWithFrame:(CGRect)frame atTop:(BOOL)top andRefresh_ID:(NSString *)Refresh_ID {
  ...
  if (self) {
    ...
    if (top) {
      ...
      //        我新添加的
      My_refresh_id = [Refresh_ID copy];
      NSString *time_date = [[NSUserDefaults standardUserDefaults] objectForKey:My_refresh_id];
      if ([time_date intValue] == 0) {
        _dateLabel.text = @"最后更新:--";
      } else {
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
        NSDate *date_channlid = [dateFormatter dateFromString:time_date];
        [self updateRefreshDate:date_channlid];
      }
      ...
      _arrow.contents = (id)[UIImage imageWithCGImage:[UIImage imageNamed:@"blueArrow.png"].CGImage
                                                scale:1
      ...
      _state_my_lable = [[UILabel alloc] init];
      _state_my_lable.frame = CGRectMake(100, 0, 120, 40);
      _state_my_lable.font = [UIFont boldSystemFontOfSize:17.0f];
      _state_my_lable.textColor = kTextColor;
      _state_my_lable.textAlignment = NSTextAlignmentCenter;
      _state_my_lable.backgroundColor = kPRBGColor;
      _state_my_lable.autoresizingMask = UIViewAutoresizingFlexibleWidth;
      _state_my_lable.text = NSLocalizedString(@"上拉自动加载", @"");
      [self addSubview:_state_my_lable];
    }
  }
  return self;
}
...
- (void)layouts {
  ...
  if (self.isAtTop) {
    ...
    _arrow.contents = (id)arrow.CGImage;
    _stateLabel.text = NSLocalizedString(@"上拉自动加载", @"");
  }
  ...
  if (_state != state) {
    ...
    if (_state == kPRStateLoading) { // Loading
      ...
      if (self.isAtTop) {
        ...
        [activityView startAnimating];
        _state_my_lable.text = NSLocalizedString(@"正在加载", @"");
      }
...
- (void)updateRefreshDate:(NSDate *)date {
  ...
  if ([dateString length] == 0) {
    _dateLabel.text = [NSString stringWithFormat:@"%@: %@", NSLocalizedString(@"最后更新", @""), @"--"];
    return;
  }

  _dateLabel.text =
      [NSString stringWithFormat:@"%@: %@", NSLocalizedString(@"最后更新", @""),
                                 [[DateChangeSimple sharedManager] DateFormaterForStr:dateString]];
}
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style {
  ...
  if (self) {
    ...
    //        _headerView = [[PullLoadingView alloc] initWithFrame:rect
    //        atTop:YES andRefresh_ID:myRefresh_id];
    _headerView =
        [[PullLoadingView alloc] initWithFrame:rect
                                         atTop:YES
                                 andRefresh_ID:[NSString stringWithFormat:@"NewsListRefrash_%@", _myRefresh_id]];
    _headerView.atTop = YES;
    [self addSubview:_headerView];

    rect = CGRectMake(0, frame.size.height, frame.size.width, frame.size.height);
    _footerView = [[PullLoadingView alloc]
        initWithFrame:rect
                atTop:NO
        andRefresh_ID:[NSString stringWithFormat:@"NewsListRefrash_bottom_%@", _myRefresh_id]];
...
- (NSDate *)get_repeat_refrash_time:(NSString *)string {
  NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
  [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
  NSDate *date_channlid = [dateFormatter dateFromString:string];

  return date_channlid;
}
...
- (void)tableViewDidScroll:(UIScrollView *)scrollView {
  ...
  //    //    我新添加的
  NSString *refrash_time = [[NSUserDefaults standardUserDefaults]
      objectForKey:[NSString stringWithFormat:@"NewsListRefrash_%@", _myRefresh_id]];

  NSString *refrash_bottom_time = [[NSUserDefaults standardUserDefaults]
      objectForKey:[NSString stringWithFormat:@"NewsListRefrash_bottom_%@", _myRefresh_id]];

  if (offset.y < 0) {
    [_headerView updateRefreshDate:[self get_repeat_refrash_time:refrash_time]];
  } else if (offset.y > 0) {
    [_footerView updateRefreshDate:[self get_repeat_refrash_time:refrash_bottom_time]];
  }
}

#pragma mark - 我新添加的，用途是:在tableview的数据为空时，隐藏，上啦刷新的，图片和文字
...
#pragma mark - 我新添加的，对应，上面的隐藏的，底视图
//我新添加的，对应，上面的隐藏的，底视图
...
- (void)tableViewDidEndDragging:(UIScrollView *)scrollView {
  ...
  if (_headerView.state == kPRStatePulling) {
    ...
    if (_pullingDelegate &&
        [_pullingDelegate respondsToSelector:@selector(pullingTableViewDidStartRefreshing:)]) {
...
- (void)tableViewDidFinishedLoadingWithMessage:(NSString *)msg {
  ...
  if (_headerView.loading) {
    ...
    NSString *refrash_time = [[NSUserDefaults standardUserDefaults]
        objectForKey:[NSString stringWithFormat:@"NewsListRefrash_%@", _myRefresh_id]];
    NSDate *date = [self get_repeat_refrash_time:refrash_time];
    if (_pullingDelegate &&
        [_pullingDelegate respondsToSelector:@selector(pullingTableViewRefreshingFinishedDate)]) {
  ...
  //    if (_footerView.state == kPRStateLoading) {
    ...
    NSString *refrash_bottom_time = [[NSUserDefaults standardUserDefaults]
        objectForKey:[NSString stringWithFormat:@"NewsListRefrash_bottom_%@", _myRefresh_id]];
    NSDate *date = [self get_repeat_refrash_time:refrash_bottom_time];
    if (_pullingDelegate &&
        [_pullingDelegate respondsToSelector:@selector(pullingTableViewLoadingFinishedDate)]) {
      date = [_pullingDelegate pullingTableViewLoadingFinishedDate];
    }
