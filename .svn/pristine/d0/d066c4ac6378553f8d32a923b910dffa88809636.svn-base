...
@implementation ProvidentFundLoans {
...
- (void)start {
  ...
  NSString *path_2 = [[NSBundle mainBundle] pathForResource:@"公积金年利率.plist" ofType:nil];
...
// UITextField 的代理方法，实现输入框中输入字数的限制
...
- (void)Calculation {
  //首先判断输入框中有无输入内容
  //如果Push_money_textfield中没有输入内容，弹出提示“请输入符合条件的贷款总金额”
  ...
  ///还款期数
  double Repayment_num = [self Get_deadline:Time_num] * 1.0;
  NSString *Repayment_num_str = [NSString stringWithFormat:@"%0.0lf期", Repayment_num];
  ///月利率
  ...
  if (YLselectBtn.row == 0) {
    ...
    double Each_payments_float = (Lump_sum * month_Rate * pow((1 + month_Rate), Repayment_num)) /
                                 (pow((1 + month_Rate), Repayment_num) - 1);
    ...
    NSString *Each_payments = [NSString stringWithFormat:@"%0.2f元", Each_payments_float];

    /// 支付利息
    NSString *Total_interest =
        [NSString stringWithFormat:@"%0.2f元", Lump_sum * month_Rate * Repayment_num -
                                                   (Lump_sum / Repayment_num * month_Rate) *
                                                       (0 + Repayment_num - 1) * Repayment_num / 2.0];

    ///每月递减
    double monthDecress = Lump_sum / Repayment_num * month_Rate;
    NSString *monthDecressStr = [NSString stringWithFormat:@"%.2f元", monthDecress];

    /// 还款总额
    NSString *The_total_repayment =
        [NSString stringWithFormat:@"%0.2d元", [Total_interest intValue] + Lump_sum];
...
- (float)The_actual_lending_rates:(int)num {
  if (num == 0) {
    /// 贷款利率 (五年）
    double Rate_float = (double)self.Fund_rate_5Below * [textFieldForMultiple.text doubleValue];
    return [self notRounding:Rate_float afterPoint:2];
  } else {

    /// 贷款利率（五年以上）
    double Rate_float = self.Fund_rate_5Above * [textFieldForMultiple.text doubleValue];
...
- (void)Rate_click_btn:(UIButton *)sender {
  if (Rate_PickerView == nil) {
    Rate_PickerView = [[My_PickerView alloc] initWithFrame:[[self superview] superview].bounds];
    Rate_PickerView.Main_datePicker.tag = 3000;
    Rate_PickerView.Title_datePicker.text = @"存款类型";
    ...
    if ([YG_Rate count] >= 1) {
      Year_Rate_num = 0;
      Rate_textfield_btn.city_label.text = YG_Rate[0];
    }
  } else {
