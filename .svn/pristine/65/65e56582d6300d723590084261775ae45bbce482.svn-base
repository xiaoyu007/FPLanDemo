...
- (NSString *)gtm_stringByEscapingHTMLUsingTable:(HTMLEscapeMap *)table
  ...
  NSMutableData *data2 =
      [NSMutableData dataWithCapacity:sizeof(unichar) * length];
  ...
  if (!buffer) {
    ...
    NSMutableData *data =
        [NSMutableData dataWithLength:length * sizeof(UniChar)];
    if (!data) {
  ...
  for (NSUInteger i = 0; i < length; ++i) {
    HTMLEscapeMap *val =
        bsearch(&buffer[i], table, size / sizeof(HTMLEscapeMap),
                sizeof(HTMLEscapeMap), EscapeMapCompare);
    if (val || (escapeUnicode && buffer[i] > 127)) {
      if (buffer2Length) {
        CFStringAppendCharacters((__bridge CFMutableStringRef)finalString, buffer2,
                                 buffer2Length);
        buffer2Length = 0;
      }
      if (val) {
        ...
        //				_GTMDevAssert(escapeUnicode && buffer[i] >
        //127, @"Illegal Character");
...
- (NSString *)gtm_stringByUnescapingFromHTML {
  ...
  NSRange subrange =
      [self rangeOfString:@"&" options:NSBackwardsSearch range:range];
  ...
  do {
    NSRange semiColonRange =
        NSMakeRange(subrange.location, NSMaxRange(range) - subrange.location);
    ...
    NSRange escapeRange = NSMakeRange(
        subrange.location, semiColonRange.location - subrange.location + 1);
    ...
    if (length > 3 && length < 11) {
      if ([escapeString characterAtIndex:1] == '#') {
        ...
        if (char2 == 'x' || char2 == 'X') {
          // Hex escape squences &#xa3;
          NSString *hexSequence =
              [escapeString substringWithRange:NSMakeRange(3, length - 4)];
          ...
          if ([scanner scanHexInt:&value] && value < USHRT_MAX && value > 0 &&
            ...
            unichar uchar = value;
            NSString *charString =
                [NSString stringWithCharacters:&uchar length:1];
          ...
          // Decimal Sequences &#123;
          NSString *numberSequence =
              [escapeString substringWithRange:NSMakeRange(2, length - 3)];
        ...
        // "standard" sequences
        for (unsigned i = 0;
             i < sizeof(gAsciiHTMLEscapeMap) / sizeof(HTMLEscapeMap); ++i) {
          if ([escapeString
