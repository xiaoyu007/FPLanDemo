...
- (void)addRecord:(NSArray *)record {
  ...
  for (uint i = 0; i < record.count; i++) {
    float colWidth =
        [self->columnsWidths[i] floatValue]; // colwidth as given
                                                            // at setup
    ...
    col1.backgroundColor = [UIColor clearColor];
      col1.textColor = [Globle colorFromHexRGB:@"8c8c8c"];
      [col1.layer
          setBorderColor:[[UIColor colorWithWhite:0.821 alpha:1.000] CGColor]];
  ...
  for (uint i = 0; i < labels.count; i++) {
    UILabel *tempLabel = (UILabel *) labels[i];
