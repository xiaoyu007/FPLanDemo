...
- (void)__startGlowing {
  ...
  UIImage *image = nil;
  CGSize imageSize = CGSizeMake(self.bounds.size.width + glowSpread,
                                self.bounds.size.height + glowSpread);
                                         ...
                                         [UIScreen mainScreen].scale);
  {
    ...
    CGGradientRef gradient =
        [[self class] __newGlowGradientWithColor:self.highlightedGlowColor];

    CGPoint gradCenter = CGPointMake(floorf(imageSize.width / 2.0f),
                                     floorf(imageSize.height / 2.0f));
    CGFloat gradRadius = MAX(imageSize.width, imageSize.height) / 2.0f;

    CGContextDrawRadialGradient(context, gradient, gradCenter, 0.0f, gradCenter,
                                gradRadius,
...
+ (CGGradientRef)__newGlowGradientWithColor:(UIColor *)color {
  ...
  CGGradientRef gradient = CGGradientCreateWithColorComponents(
      colorSpace, colorComponents, locations, locationsCount);
