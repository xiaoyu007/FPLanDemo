...
- (void)setUp {
  ...
  //    self.overlayColor = [UIColor colorWithRed:1.0 green:0. blue:0.
  //    alpha:1.0];
...
- (void)drawRect:(CGRect)rect {
  ...
  if (_progress < 1.) {
    ...
    CGPathAddArc(path2, &transform, 0., 0., innerRadius, 0.,
                 angle / 180. * M_PI, 0.);
...
- (void)setInnerRadiusRatio:(CGFloat)innerRadiusRatio {
  _innerRadiusRatio = (innerRadiusRatio < 0.) ? 0. : (innerRadiusRatio > 1.)
                                                         ? 1.
                                                         : innerRadiusRatio;
}

- (void)setOuterRadiusRatio:(CGFloat)outerRadiusRatio {
  _outerRadiusRatio = (outerRadiusRatio < 0.) ? 0. : (outerRadiusRatio > 1.)
                                                         ? 1.
                                                         : outerRadiusRatio;
}
  ...
  if (_progress != progress) {
    ...
    if (progress > 0. && progress < 1.) {
      ...
      [self setNeedsDisplay];
               ...
               self.triggersDownloadDidFinishAnimationAutomatically) {
...
- (CGFloat)innerRadius {
  ...
  case DAProgressOverlayViewStateOperationFinished:
    return radius +
           (MAX(width, height) / sqrtf(2.) - radius) * self.animationProggress;
  default:
    return radius;
  }
}
...
- (void)update {
  CGFloat animationProggress =
      self.animationProggress +
      DAUpdateUIFrequency / self.stateChangeAnimationDuration;
