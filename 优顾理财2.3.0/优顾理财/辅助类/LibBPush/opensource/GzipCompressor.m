...
- (NSError *)setupStream {
  ...
  zStream.avail_in = 0;
  zStream.next_in = 0;
  int status = deflateInit2(&zStream, COMPRESSION_AMOUNT, Z_DEFLATED, (15 + 16),
...
- (NSData *)compressBytes:(Bytef *)bytes
  ...
  // We'll take a guess that the compressed data will fit in half the size of
  // the original (ie the max to compress at once is half DATA_CHUNK_SIZE), if
  // not, we'll increase it below
  ...
  while (zStream.avail_out == 0) {
    ...
    zStream.next_out = (Bytef *)[outputData mutableBytes] + zStream.total_out -
                       bytesProcessedAlready;
    zStream.avail_out =
        (unsigned int)([outputData length] -
...
+ (NSData *)compressData:(NSData *)uncompressedData {
  NSData *outputData = [[GzipCompressor compressor]
      compressBytes:(Bytef *)[uncompressedData bytes]
