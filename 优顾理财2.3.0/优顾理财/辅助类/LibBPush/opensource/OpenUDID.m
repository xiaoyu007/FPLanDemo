...
//#define OpenUDIDLog(fmt, ...) NSLog((@"%s [Line %d] " fmt),
//__PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
//#define OpenUDIDLog(fmt, ...) NSLog((@"[Line %d] " fmt), __LINE__,
//##__VA_ARGS__);
...
+ (void)_setDict:(id)dict forPasteboard:(id)pboard {
  ...
  [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict]
      forPasteboardType:kOpenUDIDDomain];
#else
  [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict]
          forType:kOpenUDIDDomain];
#endif
}
...
+ (NSMutableDictionary *)_getDictFromPasteboard:(id)pboard {
  ...
  return [NSMutableDictionary
      dictionaryWithDictionary:(item == nil ||
...
// Theoretically, this function is called once ever per application when calling
// [OpenUDID value] for the first time.
// After that, the caching/pasteboard/redundancy mechanism inside [OpenUDID
// value] returns a persistent and cross application OpenUDID
//
+ (NSString *)_generateFreshOpenUDID {

  NSString *_openUDID = nil;

// August 2011: One day, this may no longer be allowed in iOS. When that is,
// just comment this line out.
// March 25th 2012: this day has come, let's remove this "outlawed" call...
// August 2012: well, perhaps much ado about nothing; in any case WWDC2012 gave
// us something to work with; read below
#if TARGET_OS_IPHONE
...
//    }
  ...
  // !!!!! IMPORTANT README !!!!!
  //
  // August 2012: iOS 6 introduces new APIs that help us deal with the now
  // deprecated [UIDevice uniqueIdentifier]
  // Since iOS 6 is still pre-release and under NDA, the following piece of code
  // is meant to produce an error at
  // compile time. Accredited developers integrating OpenUDID are expected to
  // review the iOS 6 release notes and
  // documentation, and replace the underscore ______ in the last part of the
  // selector below with the right
  // selector syntax as described here (make sure to use the right one! last
  ...
  //
  // The semantic compiler warnings are still normal if you are compiling for
  // iOS 5 only since Xcode will not
  // know about the two instance methods used on that line; the code running on
  // iOS will work well at run-time.
  // Either way, it's time that you junped on the iOS 6 bandwagon and start
  // testing your code on iOS 6 ;-)
  //
  // WHAT IS THE SIGNIFICANCE OF THIS CHANGE?
  //
  // Essentially, this means that OpenUDID will keep on behaving the same way as
  // before for existing users or
  // new users in iOS 5 and before. For new users on iOS 6 and after, the new
  // official public APIs will take over.
  // OpenUDID will therefore be obsoleted when iOS reaches significant adoption,
  // anticipated around mid-2013.

  /*

      September 14; ok, new development. The alleged API has moved!
      This part of the code will therefore be updated when iOS 6 is actually
released.
      Nevertheless, if you want to go ahead, the code should be pretty easy to
      guess... it involves including a .h file from a nine-letter framework that
ends
      with the word "Support", and then assigning _openUDID with the only
identifier method called on the sharedManager of that new class... don't forget
to add
...
# error                                                         ^ read comments
above, fix accordingly, and remove this #error line
  }
#endif
  ...
  // Unique Identifiers) or IIDs
  // (Interface Identifiers), are 128-bit values guaranteed to be unique. A UUID
  // is made unique over
  // both space and time by combining a value unique to the computer on which it
  // was generated—usually the
  ...
  // We then hash this UUID with md5 to get 32 bytes, and then add 4 extra
  // random bytes
  ...
  // NOTE that the goal is NOT to find a better hash method, but rather, find a
  // decentralized (i.e. not web-based)
...
// typically the first time it is called
// It will return the null udid (forty zeros) if the user has somehow opted this
// app out (this is subject to 3rd party implementation)
...
+ (NSString *)valueWithError:(NSError **)error {
          ...
          [NSError errorWithDomain:kOpenUDIDDomain
                          ...
                          userInfo:[NSDictionary
                                       dictionaryWithObjectsAndKeys:
                                           @"OpenUDID in cache from first call",
                                           @"description", nil]];
  ...
  // This local copy contains a copy of the openUDID, myRedundancySlotPBid (and
  // unused in this block, the local bundleid, and the timestamp)
  //
  id localDict = [defaults objectForKey:kOpenUDIDKey];
  if ([localDict isKindOfClass:[NSDictionary class]]) {
    localDict = [NSMutableDictionary
        dictionaryWithDictionary:localDict]; // we might need to set/overwrite
                                             // the redundancy slot
  ...
  // Here we go through a sequence of slots, each of which being a UIPasteboard
  // created by each participating app
  // The idea behind this is to both multiple and redundant representations of
  // OpenUDIDs, as well as serve as placeholder for potential opt-out
  //
  NSString *availableSlotPBid = nil;
  NSMutableDictionary *frequencyDict =
      [NSMutableDictionary dictionaryWithCapacity:kOpenUDIDRedundancySlots];
  for (int n = 0; n < kOpenUDIDRedundancySlots; n++) {
    NSString *slotPBid =
        [NSString stringWithFormat:@"%@%d", kOpenUDIDSlotPBPrefix, n];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
    ...
    if (slotPB == nil) {
      ...
      if (oudid == nil) {
        // availableSlotPBid could inside a non null slot where no oudid can be
        // found
        if (availableSlotPBid == nil)
          availableSlotPBid = slotPBid;
      } else {
      ...
      // then let's look if the external UIPasteboard representation marks this
      // app as OptedOut
      NSString *gid = [dict objectForKey:kOpenUDIDAppUIDKey];
      if (gid != nil && [gid isEqualToString:appUID]) {
        myRedundancySlotPBid = slotPBid;
        // the local dictionary is prime on the opt-out subject, so ignore if
        // already opted-out locally
        if (optedOut) {
  ...
  // sort the Frequency dict with highest occurence count of the same OpenUDID
  // (redundancy, failsafe)
  // highest is last in the list
  //
  NSArray *arrayOfUDIDs =
      [frequencyDict keysSortedByValueUsingSelector:@selector(compare:)];
  NSString *mostReliableOpenUDID =
      (arrayOfUDIDs != nil && [arrayOfUDIDs count] > 0)
          ? [arrayOfUDIDs lastObject]
          : nil;
  OpenUDIDLog(@"Freq Dict = %@\nMost reliable %@", frequencyDict,
              mostReliableOpenUDID);

  // if openUDID was not retrieved from the local preferences, then let's try to
  // get it from the frequency dictionary above
  //
  if (openUDID == nil) {
    if (mostReliableOpenUDID == nil) {
      // this is the case where this app instance is likely to be the first one
      // to use OpenUDID on this device
      // we create the OpenUDID, legacy or semi-random (i.e. most certainly
      // unique)
      //
      openUDID = [OpenUDID _generateFreshOpenUDID];
    } else {
      // or we leverage the OpenUDID shared by other apps that have already gone
      // through the process
      //
    ...
    //
    if (mostReliableOpenUDID != nil &&
        ![mostReliableOpenUDID isEqualToString:openUDID])
      isCompromised = YES;
  }

  // Here we store in the available PB slot, if applicable
  //
  OpenUDIDLog(@"Available Slot %@ Existing Slot %@", availableSlotPBid,
              myRedundancySlotPBid);
  if (availableSlotPBid != nil &&
      (myRedundancySlotPBid == nil ||
       [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
    UIPasteboard *slotPB =
        [UIPasteboard pasteboardWithName:availableSlotPBid create:YES];
  ...
  // If the UIPasteboard external representation marks this app as opted-out,
  // then to respect privacy, we return the ZERO OpenUDID, a sequence of 40
  // zeros...
  // This is a *new* case that developers have to deal with. Unlikely,
  // statistically low, but still.
  // To circumvent this and maintain good tracking (conversion ratios, etc.),
  // developers are invited to calculate how many of their users have opted-out
    ...
    if (isCompromised)
      *error = [NSError
          errorWithDomain:kOpenUDIDDomain
                 ...
                 userInfo:[NSDictionary
                              dictionaryWithObjectsAndKeys:
                                  @"Found a discrepancy between stored "
                                  @"OpenUDID (reliable) and redundant copies; "
                                  @"one of the apps on the device is most "
                                  @"likely corrupting the OpenUDID protocol",
                                  @"description", nil]];
    else
      *error = [NSError
          errorWithDomain:kOpenUDIDDomain
                 ...
                 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:
                                            @"OpenUDID succesfully retrieved",
                                            @"description", nil]];
  }
