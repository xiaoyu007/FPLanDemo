    ...
    for (columnIdx = 0; columnIdx < columnCount; columnIdx++) {
      [_columnNameToIndexMap
          setObject:[NSNumber numberWithInt:columnIdx]
             forKey:[[NSString
                        stringWithUTF8String:sqlite3_column_name(
...
- (void)kvcMagic:(id)object {
  ...
  for (columnIdx = 0; columnIdx < columnCount; columnIdx++) {
    ...
    const char *c =
        (const char *)sqlite3_column_text([_statement statement], columnIdx);
...
- (NSDictionary *)resultDict {
  ...
  if (num_cols > 0) {
    NSMutableDictionary *dict =
        [NSMutableDictionary dictionaryWithCapacity:num_cols];
  ...
  if (num_cols > 0) {
    ...
    for (columnIdx = 0; columnIdx < columnCount; columnIdx++) {
      ...
      NSString *columnName = [NSString
          stringWithUTF8String:sqlite3_column_name([_statement statement],
                                                   columnIdx)];
...
- (BOOL)next {
  ...
  if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
    // nslog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB
    // databasePath]);
    // nslog(@"Database busy");
  } else if (SQLITE_DONE == rc || SQLITE_ROW == rc) {
    // all is well, let's return.
  } else if (SQLITE_ERROR == rc) {
    // nslog(@"Error calling sqlite3_step (%d: %s) rs", rc,
    ...
    // wtf?
    // nslog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc,
    // sqlite3_errmsg([_parentDB sqliteHandle]));
  }
...
- (unsigned long long int)unsignedLongLongIntForColumn:(NSString *)columnName {
  return [self
      unsignedLongLongIntForColumnIndex:[self columnIndexForName:columnName]];
}
...
- (NSDate *)dateForColumnIndex:(int)columnIdx {
  ...
  return [_parentDB hasDateFormatter]
             ...
             : [NSDate dateWithTimeIntervalSince1970:
                           [self doubleForColumnIndex:columnIdx]];
}
...
- (NSData *)dataForColumnIndex:(int)columnIdx {
  ...
  memcpy([data mutableBytes],
         sqlite3_column_blob([_statement statement], columnIdx), dataSize);
...
- (NSData *)dataNoCopyForColumnIndex:(int)columnIdx {
  ...
  NSData *data =
      [NSData dataWithBytesNoCopy:(void *)sqlite3_column_blob(
...
- (id)objectForColumnIndex:(int)columnIdx {
  ...
  if (columnType == SQLITE_INTEGER) {
    returnValue = [NSNumber
        numberWithLongLong:[self longLongIntForColumnIndex:columnIdx]];
  } else if (columnType == SQLITE_FLOAT) {
    returnValue =
        [NSNumber numberWithDouble:[self doubleForColumnIndex:columnIdx]];
...
// returns autoreleased NSString containing the name of the column in the result
// set
- (NSString *)columnNameForIndex:(int)columnIdx {
  return [NSString stringWithUTF8String:sqlite3_column_name(
                                            [_statement statement], columnIdx)];
}
