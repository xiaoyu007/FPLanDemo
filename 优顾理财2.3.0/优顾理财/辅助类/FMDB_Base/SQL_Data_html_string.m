...
static SQL_Data_html_string *_SQL_Data_html_string = nil;

@implementation SQL_Data_html_string
  ...
  if ([[NSFileManager defaultManager]
          fileExistsAtPath:pathInCacheDirectory([NSString
                               stringWithFormat:@"/Collection.xmly/%@",
                                                kDefaultDBName])] == NO) {
    _SQL_Data_html_string = nil;
  }
  @synchronized([SQL_Data_html_string class]) {
    if (!_SQL_Data_html_string)
      _SQL_Data_html_string = [[self alloc] init];
    return _SQL_Data_html_string;
  }
...
+ (id)alloc {
  ...
  @synchronized([SQL_Data_html_string class]) {
    NSAssert(_SQL_Data_html_string == nil,
             @"Attempted to allocated a second instance_sql_data_html_string");
    _SQL_Data_html_string = [super alloc];
    return _SQL_Data_html_string;
  }
  return nil;
}
...
- (void)createDataBase {
  ...
  if ([set1 intForColumnIndex:0]) {
    ...
    [_db executeUpdate:@"CREATE TABLE PUser (uid INTEGER PRIMARY KEY "
                       @"AUTOINCREMENT  NOT NULL, name VARCHAR(50), praise "
                       @"VARCHAR(2))"];
  }
}
/**
...
- (BOOL)View_user_exists:(NSString *)name andChannlid:(NSString *)channlid {
  ...
  //定义一个结果集，存放查询的数据
  FMResultSet *rs =
      [_db executeQuery:[NSString stringWithFormat:@"select Fid from SUser "
                                                   @"WHERE name = '%@' and "
                                                   @"channlid = '%@'",
                                                   name, channlid]];
...
- (void)mergeWithUser:(NSString *)name
  ...
  NSString *query =
      [NSString stringWithFormat:@"UPDATE SUser set add_cancle=%@ and "
...
- (void)saveUser:(NSString *)name
  ...
  NSMutableString *query =
      [NSMutableString stringWithFormat:@"INSERT INTO SUser"];
...
- (void)deleteUserWithId:(NSString *)uid andChannlid:(NSString *)channlid {
  NSString *query =
      [NSString stringWithFormat:
                    @"DELETE FROM SUser WHERE name = '%@' and channlid = '%@'",
                    uid, channlid];
...
- (BOOL)PUser_zan:(NSString *)name {
  FMResultSet *rs = [_db
      executeQuery:
          [NSString
              stringWithFormat:@"select * from PUser WHERE name = '%@'", name]];
...
- (void)save_PUser:(NSString *)name {
  NSMutableString *query =
      [NSMutableString stringWithFormat:@"INSERT INTO  PUser"];
  NSMutableString *keys = [NSMutableString stringWithFormat:@" ("];
  NSMutableString *values = [NSMutableString stringWithFormat:@" ( "];
  NSMutableArray *arguments = [NSMutableArray arrayWithCapacity:5];
  if (name) {
...
- (void)delete_PUser:(NSString *)name {
  NSString *query =
      [NSString stringWithFormat:@"DELETE FROM PUser WHERE name = '%@'", name];
