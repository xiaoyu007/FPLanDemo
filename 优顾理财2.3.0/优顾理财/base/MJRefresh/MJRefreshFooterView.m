...
#define kPullToRefresh @"上拉可以加载"
#define kReleaseToRefresh @"松开立即加载"
#define kRefreshing @"努力加载中..."
//#define kForbidRefresh @"暂无更多数据"
...
- (id)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    ...
    tipLabel = label;
    // tipLabel.text = kForbidRefresh;
    tipLabel.center = CGPointMake(170, self.validY + kViewHeight / 2);
    tipLabel.backgroundColor = [UIColor clearColor];
    CGFloat color = 178 / 255.0;
    tipLabel.textColor =
        [UIColor colorWithRed:color green:color blue:color alpha:1.0];
...
- (void)adjustFrame {
  ...
  if (bSingleRow) {
    [super resetOneLabelFrame];
  }
}

- (void)forbidReload:(BOOL)breload {
  _lastUpdateTimeLabel.hidden = breload;
  _statusLabel.hidden = breload;
  _arrowImage.hidden = breload;
  _activityView.hidden = YES;
  tipLabel.hidden = !breload;
}

//-(void)resetOneLabelFrame
//{
//    _statusLabel.center = tipLabel.center;
  ...
  case RefreshStateRefreshing: {
    ...
    [UIView animateWithDuration:0.2
                     animations:^{
                         // 1.顶部多出65的滚动范围
                         UIEdgeInsets inset = _scrollView.contentInset;
                         inset.bottom = self.frame.origin.y -
                         ...
                         _scrollView.contentOffset =
                             CGPointMake(0, self.validY + kViewHeight);
                     }];
    break;
  }
  }
