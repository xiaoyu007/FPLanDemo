...
- (void)initial {
  ...
  // self.backgroundColor = [UIColor colorWithRed:color green:color blue:color
  // alpha:1];
...
- (void)setFrame:(CGRect)frame {
  ...
  if (frame.size.width == 0 || _statusLabel.frame.origin.y == statusY)
    return;
  ...
  _lastUpdateTimeLabel.frame =
      CGRectMake(statusX, lastUpdateY, statusWidth, statusHeight);
  // 3.箭头
  CGFloat arrowX = 20;
...
- (void)resetOneLabelFrame {
  CGPoint labelPoint;
  labelPoint.x = self.frame.size.width / 2;
  labelPoint.y = _arrowImage.center.y;
  _statusLabel.center = labelPoint;
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if ([@"contentOffset" isEqualToString:keyPath]) {
    ...
    if (!self.userInteractionEnabled || self.alpha <= 0.01 || self.hidden ||
        _state == RefreshStateRefreshing || offsetY <= validY)
      return;
