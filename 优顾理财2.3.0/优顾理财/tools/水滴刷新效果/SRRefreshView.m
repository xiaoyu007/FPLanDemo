...
- (id)initWithHeight:(CGFloat)height {
  ...
  if (self) {
    ...
    _refleshView =
        [[UIImageView alloc] initWithImage:nil];//@"sr_refresh"
    _refleshView.center = _slime.startPoint;
    _refleshView.bounds =
        CGRectMake(0.0f, 0.0f, kRefreshImageWidth, kRefreshImageWidth);
...
- (void)setLoading:(BOOL)loading {
  ...
  if (_loading) {
    ...
    CAKeyframeAnimation *aniamtion =
        [CAKeyframeAnimation animationWithKeyPath:@"transform"];

    aniamtion.values = @[[NSValue
        valueWithCATransform3D:CATransform3DRotate(
...
- (void)restore {
  ...
  [UIView transitionWithView:_activityIndicatorView
      ...
      completion:^(BOOL finished) {
        ...
        //                                    CATransform3DMakeScale(0.1, 0.1,
        //                                    1)];
