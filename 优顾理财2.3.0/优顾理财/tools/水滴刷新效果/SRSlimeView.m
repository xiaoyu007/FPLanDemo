...
NS_INLINE CGPoint
pointLineToArc(CGPoint center, CGPoint p2, float angle, CGFloat radius) {
...
- (UIBezierPath *)bodyPath:(CGFloat)startRadius
                       end:(CGFloat)endRadius
...
#define kMiddleP 0.6
  CGPoint mp1 = CGPointMake(sp2.x * kMiddleP + ep1.x * (1 - kMiddleP),
                            sp2.y * kMiddleP + ep1.y * (1 - kMiddleP)),
          mp2 = CGPointMake(sp1.x * kMiddleP + ep2.x * (1 - kMiddleP),
                            sp1.y * kMiddleP + ep2.y * (1 - kMiddleP)),
  ...
  case SRSlimeStateNormal: {
    ...
    if (percent == 1) {
      ...
      UIBezierPath *path = [UIBezierPath
          bezierPathWithRoundedRect:CGRectMake(_startPoint.x - _radius,
      ...
      UIBezierPath *path =
          [self bodyPath:startRadius end:endRadius percent:percent];
      [self setContext:context path:path];
      CGContextDrawPath(context, kCGPathFillStroke);
      if (percent <= 0) {
...
#pragma clang diagnostic pop
  ...
  case SRSlimeStateShortening: {
    ...
    if (p > 0.01) {
      ...
      CGFloat endRadius =
          r * (kEndTo + (1 - kEndTo) * percent) * (1 + percent / 2);
      UIBezierPath *path =
          [self bodyPath:startRadius end:endRadius percent:percent];
      [self setContext:context path:path];
      CGContextDrawPath(context, kCGPathFillStroke);
...
- (void)setContext:(CGContextRef)context path:(UIBezierPath *)path {
  if (self.shadowColor) {
    ...
    case SRSlimeBlurShadow:
      CGContextSetShadowWithColor(context, CGSizeZero, self.shadowBlur,
